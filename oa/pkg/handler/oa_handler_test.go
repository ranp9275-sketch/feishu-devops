...
func TestStoreJsonHandler(t *testing.T) {
	...
	t.Run("Valid JSON", func(t *testing.T) {
		...
		reqBody := `{"key": "value"}`
		c.Request = httptest.NewRequest(http.MethodPost, "/api/store-json-handler", bytes.NewBufferString(reqBody))
...
func TestGetJsonBatchHandler(t *testing.T) {
	...
	t.Run("Should return 404 if no valid files", func(t *testing.T) {
		// Clean up valid file
